...
class ZoomedInCGImage {
    ...
    func zoomedInImage() -> UIImage? {
        guard
            let topLayer = createResizeLayer(
                yMin: 0.0,
                imageHeight: imageCenterMinY,
                cropHeight: cropQuarterHeight
            )
        else { return nil }
        guard
            let midLayer = createResizeLayer(
                yMin: imageCenterMinY,
                imageHeight: cropQuarterHeight * 2,
                cropHeight: cropQuarterHeight * 2
            )
        else { return nil }
        guard
            let bottomLayer = createResizeLayer(
                yMin: imageCenterMaxY,
                imageHeight: imageCenterMinY,
                cropHeight: cropQuarterHeight
            )
        else { return nil }
        ...
        midLayer.draw(
            in: CGRect(
                ...
                height: cropQuarterHeight * 2
            )
        )
        bottomLayer.draw(
            in: CGRect(
                x: 0.0,
                y: cropQuarterHeight * 3,
                width: finalCropWidth,
                height: cropQuarterHeight
            )
        )
    ...
    private func createResizeLayer(
        yMin: CGFloat,
        imageHeight: CGFloat,
        cropHeight: CGFloat
    ) -> UIImage? {
        let leftCropRect = CGRect(x: 0.0, y: yMin, width: imageCenterMinX, height: imageHeight)
        let midCropRect = CGRect(
            x: imageCenterMinX,
            y: yMin,
            width: cropQuarterWidth * 2,
            height: imageHeight
        )
        let rightCropRect = CGRect(
            x: imageCenterMaxX,
            y: yMin,
            width: imageCenterMinX,
            height: imageHeight
        )

        guard let leftCropImage = self.image.cropping(to: leftCropRect),
            let leftResizedImage = resize(
                image: leftCropImage,
                targetSize: CGSize(width: cropQuarterWidth, height: cropHeight)
            )
        else { return nil }

        guard let midCropImage = self.image.cropping(to: midCropRect),
            let midResizedImage = resize(
                image: midCropImage,
                targetSize: CGSize(width: cropQuarterWidth * 2, height: cropHeight)
            )
        else { return nil }

        guard let rightCropImage = self.image.cropping(to: rightCropRect),
            let rightResizedImage = resize(
                image: rightCropImage,
                targetSize: CGSize(width: cropQuarterWidth, height: cropHeight)
            )
        else { return nil }
        ...
        leftResizedImage.draw(
            in: CGRect(x: 0.0, y: 0.0, width: cropQuarterWidth, height: cropHeight)
        )
        midResizedImage.draw(
            in: CGRect(x: cropQuarterWidth, y: 0.0, width: cropQuarterWidth * 2, height: cropHeight)
        )
        rightResizedImage.draw(
            in: CGRect(x: cropQuarterWidth * 3, y: 0.0, width: cropQuarterWidth, height: cropHeight)
        )
