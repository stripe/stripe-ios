...
class ImageCompressionTests: XCTestCase {
    ...
    func testFullSize() throws {
        guard let image = image, let originalImageSize = originalImageSize,
            let roiRectangle = roiRectangle
        else {
            throw "invalid setup"
        }

        let scannedCard = ScannedCardImageData(
            previewLayerImage: image,
            previewLayerViewfinderRect: roiRectangle
        )
    ...
    func testJPEG() throws {
        ...
        let (verificationFrame, metadata) = scannedCard.toVerificationFramesData(
            imageConfig: ImageConfig(preferredFormats: [.jpeg])
        )
        let imageData = verificationFrame.imageData
        let newImage = UIImage(data: imageData!)
    ...
    func testHEIC() throws {
        ...
        let (verificationFrame, metadata) = scannedCard.toVerificationFramesData(
            imageConfig: ImageConfig(preferredFormats: [.heic])
        )
        let imageData = verificationFrame.imageData
        let newImage = UIImage(data: imageData!)
...
extension ViewFinderMargins {
    func equal(to rect: CGRect) -> Bool {
        ...
        return left == self.left && right == self.right && upper == self.upper
            && lower == self.lower
    }
}
