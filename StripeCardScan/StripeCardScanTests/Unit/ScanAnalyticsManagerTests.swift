...
class ScanAnalyticsManagerTests: XCTestCase {
    ...
    override func setUp() {
        ...
        self.generatePayloadExp = expectation(
            description: "Successfully generated the scan analytics payload"
        )
    }
    ...
    func testGeneratePayload() {
        ...
        let payloadInfo = ScanAnalyticsPayload.PayloadInfo(
        ...
        scanAnalyticsManager.generateScanAnalyticsPayload(with: .init()) {
            [weak self] scanAnalyticsPayload in
            ...
            XCTAssertEqual(
                payloadScanStats.tasks.cameraPermission.first?.result,
                ScanAnalyticsEvent.success.rawValue
            )
            XCTAssertEqual(
                payloadScanStats.tasks.completionLoopDuration.first?.result,
                ScanAnalyticsEvent.success.rawValue
            )
            XCTAssertEqual(
                payloadScanStats.tasks.imageCompressionDuration.first?.result,
                ScanAnalyticsEvent.success.rawValue
            )
            XCTAssertEqual(
                payloadScanStats.tasks.mainLoopDuration.first?.result,
                ScanAnalyticsEvent.success.rawValue
            )
            XCTAssertEqual(
                payloadScanStats.tasks.torchSupported.first?.result,
                ScanAnalyticsEvent.torchSupported.rawValue
            )
            XCTAssertEqual(
                payloadScanStats.tasks.scanActivity.first?.result,
                ScanAnalyticsEvent.firstImageProcessed.rawValue
            )
            XCTAssertEqual(
                payloadScanStats.repeatingTasks.mainLoopImagesProcessed,
                .init(executions: 100)
            )
        }
    ...
    func testReset() {
        ...
        scanAnalyticsManager.generateScanAnalyticsPayload(with: .init()) {
            ...
            XCTAssertNil(
                payload.payloadInfo,
                "A reset scan analytics manager should have a nil payload info"
            )
            ...
            XCTAssertEqual(
                payloadScanStats.tasks.cameraPermission.first?.result,
                ScanAnalyticsEvent.unknown.rawValue
            )
            XCTAssertEqual(
                payloadScanStats.tasks.completionLoopDuration.first?.result,
                ScanAnalyticsEvent.unknown.rawValue
            ...
            XCTAssert(
                payloadScanStats.tasks.scanActivity.isEmpty,
                "A reset scan analytics manager should have an empty scan activity list"
            )
        }
