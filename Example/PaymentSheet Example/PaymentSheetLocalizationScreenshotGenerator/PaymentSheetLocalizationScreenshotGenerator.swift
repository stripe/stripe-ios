...
class PaymentSheetLocalizationScreenshotGenerator: XCTestCase {
    ...
    func scrollToPaymentMethodCell(_ cell: String) {
        let paymentMethodTypeCollectionView = app.collectionViews["PaymentMethodTypeCollectionView"].firstMatch
...
//        waitToAppear(app.buttons["Present PaymentSheet"].firstMatch)
//
//        app.segmentedControls["mode_selector"].firstMatch.buttons["Pay"].tap() // payment intent
//        app.segmentedControls["customer_mode_selector"].firstMatch.buttons["returning"].tap() // returning customer
