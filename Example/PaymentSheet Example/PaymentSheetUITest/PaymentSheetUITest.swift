...
class PaymentSheetStandardUITests: PaymentSheetUITestCase {
    ...
    func testPaymentSheetCustomDeferred_update() throws {
        ...
        let subscribeSwitch = app.switches["subscribe_switch"].firstMatch
        ...
        app.switches["subscribe_switch"].firstMatch.tap()
        app.steppers["hotdog_stepper"].firstMatch.tap()
        app.steppers["hotdog_stepper"].firstMatch.tap()
        app.steppers["salad_stepper"].firstMatch.tap()
        ...
        let saladStepper = app.steppers["salad_stepper"].firstMatch
    ...
    func testPaymentSheetCustomSaveAndRemoveCard() throws {
        ...
        var saveThisCardToggle = app.switches["Save this card for future Example, Inc. payments"].firstMatch
        ...
        saveThisCardToggle = app.switches["Save this card for future Example, Inc. payments"].firstMatch
        saveThisCardToggle.tap()
        XCTAssertTrue(saveThisCardToggle.isSelected)
    ...
    func testIdealPaymentMethodHasTextFieldsAndDropdown() throws {
        ...
        let webviewCloseButton = app.otherElements["TopBrowserBar"].firstMatch.buttons["Close"]
...
//        app.buttons["COMPLETE"].firstMatch.waitForExistenceAndTap()
//        let successText = app.alerts.staticTexts["Your order is confirmed!"]
//        XCTAssertTrue(successText.waitForExistence(timeout: 10.0))
//        let okButton = app.alerts.scrollViews.otherElements.buttons["OK"]
//        okButton.tap()
...
class PaymentSheetStandardLPMUITests: PaymentSheetUITestCase {
    ...
    func testKlarnaPaymentMethod() throws {
        ...
        let webviewCloseButton = app.buttons["Cancel"].firstMatch
        XCTAssertTrue(webviewCloseButton.waitForExistence(timeout: 10.0))
        webviewCloseButton.tap()
    }
    ...
    func testCashAppPaymentMethod() throws {
        ...
        XCTAssertTrue(app.staticTexts["The customer declined this payment."].firstMatch.waitForExistence(timeout: 5.0))
        ...
        XCTAssertTrue(app.staticTexts["Success!"].firstMatch.waitForExistence(timeout: 15.0))
    }
    ...
    func testCashAppPaymentMethod_setup() throws {
        ...
        app.buttons["Present PaymentSheet"].firstMatch.tap()
        let setupButton = app.buttons["Set up"].firstMatch
    ...
    func testUSBankAccountPaymentMethod() throws {
        ...
        let saveThisAccountToggle = app.switches["Save this account for future Example, Inc. payments"].firstMatch
    ...
    func testUPIPaymentMethod() throws {
        ...
        XCTAssertTrue(app.buttons["Pay ₹50.99"].firstMatch.waitForExistence(timeout: 5))
    ...
    func testCardBrandChoiceWithPreferredNetworks() throws {
        ...
        // We should have selected Visa due to preferreedNetworks configuration API
        XCTAssertTrue(app.textFields["Visa"].firstMatch.waitForExistence(timeout: 2))
        ...
        XCTAssertFalse(app.textFields["Select card brand (optional)"].firstMatch.waitForExistence(timeout: 2))
    ...
    func testCardBrandChoiceSavedCard() {
        ...
        XCTAssertTrue(app.textFields["Cartes Bancaires"].firstMatch.waitForExistence(timeout: 5))

        // toggle save this card on
        let saveThisCardToggle = app.switches["Save this card for future Example, Inc. payments"].firstMatch
        saveThisCardToggle.tap()
        XCTAssertTrue(saveThisCardToggle.isSelected)
        ...
        XCTAssertTrue(app.staticTexts["••••1001"].firstMatch.waitForExistence(timeout: 5.0))
        XCTAssertTrue(app.images["carousel_card_cartes_bancaires"].firstMatch.waitForExistence(timeout: 5))
        ...
        XCTAssertTrue(app.images["carousel_card_visa"].firstMatch.waitForExistence(timeout: 5))
        ...
        // Pay with this card
        app.buttons["Pay €50.99"].firstMatch.tap()
        XCTAssertTrue(app.staticTexts["Success!"].firstMatch.waitForExistence(timeout: 10.0))
        ...
        XCTAssertFalse(app.staticTexts["••••1001"].firstMatch.waitForExistence(timeout: 5.0))
    }
    ...
    func testSavedSEPADebitPaymentMethod_FlowController_ShowsMandate() {
        ...
        app.tap()
        XCTAssertTrue(app.staticTexts["Payment canceled."].firstMatch.waitForExistence(timeout: 10.0))
...
class PaymentSheetDeferredUITests: PaymentSheetUITestCase {
    ...
    func testDeferredPaymentIntent_ClientSideConfirmation() {
        ...
        app.buttons["Present PaymentSheet"].firstMatch.tap()
        XCTAssertTrue(app.buttons["Pay $50.99"].firstMatch.waitForExistence(timeout: 10))
    ...
    /* Disable Link test
     ...
     let modal = app.otherElements["Stripe.Link.PayWithLinkWebController"].firstMatch
...
class PaymentSheetGDPRUITests: PaymentSheetUITestCase {
    func testAllowRedisplayValue_unspecified_PI_legacy() {
        ...
        app.buttons["PaymentMethodRedisplayFilters, always"].firstMatch.waitForExistenceAndTap()
        app.buttons["unspecified"].firstMatch.waitForExistenceAndTap()
    ...
    func testAllowRedisplayValue_unspecified_SI_legacy() {
        ...
        XCTAssertTrue(app.buttons["Set up"].firstMatch.waitForExistence(timeout: 10))
        XCTAssertEqual(app.staticTexts.matching(identifier: "••••4242").count, 1)
        app.buttons["Close"].firstMatch.waitForExistenceAndTap()
    }
    ...
    func _testAllowRedisplayByAddingSingleCard(settings: PaymentSheetTestPlaygroundSettings,
        ...
        let paymentMethodRedisplayFilters = app.buttons["PaymentMethodRedisplayFilters, always"].firstMatch
...
class PaymentSheetDeferredUIBankAccountTests: PaymentSheetUITestCase {
    ...
    /* Disable Link test
     ...
     let modal = app.otherElements["Stripe.Link.PayWithLinkWebController"].firstMatch
     let paymentMethodPicker = app.otherElements["Stripe.Link.PaymentMethodPicker"].firstMatch
...
class PaymentSheetDeferredServerSideUITests: PaymentSheetUITestCase {
    ...
    func testExternalPaypalPaymentSheet() throws {
        ...
        XCTAssertTrue(app.staticTexts["Something went wrong!"].firstMatch.waitForExistence(timeout: 5.0))

        payButton.tap()
        app.buttons["Confirm"].firstMatch.tap()
        XCTAssertTrue(app.staticTexts["Success!"].firstMatch.waitForExistence(timeout: 5.0))
    }
    ...
    func testExternalPaypalPaymentSheetFlowController() throws {
        ...
        XCTAssertTrue(app.staticTexts["PayPal"].firstMatch.waitForExistence(timeout: 5.0))
        XCTAssertTrue(app.staticTexts["external_paypal"].firstMatch.waitForExistence(timeout: 5.0))
        ...
        payButton.tap()
        app.buttons["Fail"].firstMatch.tap()
        XCTAssertTrue(app.staticTexts["Payment failed: Error Domain= Code=0 \"Something went wrong!\" UserInfo={NSLocalizedDescription=Something went wrong!}"].firstMatch.waitForExistence(timeout: 5.0))
    ...
    func testDedupedPaymentMethods_paymentSheet() throws {
        ...
        XCTAssertFalse(app.staticTexts["Edit"].firstMatch.waitForExistence(timeout: 1))
        ...
        app.buttons["unspecified_limited_always"].firstMatch.waitForExistenceAndTap()
    ...
    func testDedupedPaymentMethods_FlowController() throws {
        ...
        XCTAssertFalse(app.staticTexts["Edit"].firstMatch.waitForExistence(timeout: 5))
        ...
        XCTAssertFalse(app.staticTexts["Edit"].firstMatch.waitForExistence(timeout: 3))
    ...
    func testRemoveLastSavedPaymentMethodPaymentSheet() throws {
        ...
        XCTAssertTrue(app.staticTexts["Done"].firstMatch.waitForExistence(timeout: 1)) // Sanity check "Done" button is there
        ...
        XCTAssertFalse(app.staticTexts["Done"].firstMatch.waitForExistence(timeout: 1)) // "Done" button is gone - we are not in edit mode
        XCTAssertFalse(app.staticTexts["Edit"].firstMatch.waitForExistence(timeout: 1)) // "Edit" button is gone - we can't edit
        XCTAssertTrue(app.buttons["Close"].firstMatch.waitForExistence(timeout: 1))
    }
    ...
    func testPSPaymentMethodRemoveTwoCards() {
        ...
        app.buttons["Back"].firstMatch.waitForExistenceAndTap(timeout: 5)
        app.buttons["Done"].firstMatch.waitForExistenceAndTap(timeout: 5)
        app.buttons["Close"].firstMatch.waitForExistenceAndTap(timeout: 5)
    }
    ...
    func testLinkOnlyFlowController() throws {
        ...
        XCTAssertTrue(app.staticTexts["Payment canceled."].firstMatch.waitForExistence(timeout: 5))

        // Re-tapping the payment method button should present the saved payment view
        app.buttons["Payment method"].firstMatch.waitForExistenceAndTap()
        XCTAssertTrue(app.staticTexts["Card information"].firstMatch.waitForExistence(timeout: 5))
    }
...
class PaymentSheetLinkUITests: PaymentSheetUITestCase {
    ...
    func testLinkPaymentSheet_disabledSPM_returningLinkUser() {
        ...
        XCTAssertTrue(app.buttons["pay_with_link_button"].firstMatch.waitForExistence(timeout: 5.0))
    ...
    func testLinkPaymentSheet_enabledSPM_hasSPMs_firstTimeLinkUser() {
        ...
        XCTAssertTrue(app.staticTexts["••••4242"].firstMatch.waitForExistence(timeout: 5.0))
        XCTAssertTrue(app.staticTexts["••••4444"].firstMatch.waitForExistence(timeout: 5.0))
    }
    ...
    func testLinkPaymentSheetFlow_disabledApplePay_enabledSPM_hasSPMs_firstTimeLinkUser() {
        ...
        XCTAssertFalse(app.buttons["pay_with_link_button"].firstMatch.waitForExistence(timeout: 5.0))
        let addCardButton = app.buttons["+ Add"].firstMatch
        XCTAssertTrue(addCardButton.waitForExistence(timeout: 4.0))
        addCardButton.tap()
        saveThisCardToggle = app.switches["Save this card for future Example, Inc. payments"].firstMatch
    ...
    func testLinkInlineSignup_gb() throws {
        ...
        app.switches["Save your info for secure 1-click checkout with Link"].firstMatch.tap()
    ...
    private func assertLinkInlineSignupNotShown() {
        ...
        XCTAssertFalse(app.switches["Save your info for secure 1-click checkout with Link"].firstMatch.waitForExistence(timeout: 2))
        // Ensure email is not shown for field consent mode
        XCTAssertFalse(app.textFields["Email"].firstMatch.waitForExistence(timeout: 3))
    }
...
extension PaymentSheetUITestCase {
    func _testUSBankAccount(mode: PaymentSheetTestPlaygroundSettings.Mode, integrationType: PaymentSheetTestPlaygroundSettings.IntegrationType, vertical: Bool = false) {
        ...
        // Select US Bank Account
        if vertical {
            XCTAssertTrue(app.buttons["US Bank Account"].firstMatch.waitForExistenceAndTap())
        } else {
        ...
        app.buttons["connect_accounts_button"].firstMatch.waitForExistenceAndTap(timeout: 10)
        ...
        switch mode {
        case .payment:
            XCTAssertTrue(app.textViews["By continuing, you agree to authorize payments pursuant to these terms."].firstMatch.waitForExistence(timeout: 5))
        case .paymentWithSetup, .setup:
            XCTAssertTrue(app.textViews["By saving your bank account for Example, Inc. you agree to authorize payments pursuant to these terms."].firstMatch.waitForExistence(timeout: 5))
        }
        ...
        reload(app, settings: settings)
        app.buttons["Present PaymentSheet"].firstMatch.tap()
        XCTAssertTrue(app.buttons["••••6789"].firstMatch.waitForExistenceAndTap())

        // Make sure bottom notice mandate is visible
        XCTAssertTrue(app.textViews["By continuing, you agree to authorize payments pursuant to these terms."].firstMatch.exists)
    ...
    func _testInstantDebits(mode: PaymentSheetTestPlaygroundSettings.Mode, vertical: Bool = false) {
        ...
        // Select "Bank"
        if vertical {
            XCTAssertTrue(app.buttons["Bank"].firstMatch.waitForExistenceAndTap())
        } else {
        ...
        // "Consent" pane
        app.buttons["Agree and continue"].firstMatch.waitForExistenceAndTap(timeout: 10)
        ...
        app.buttons["Done"].firstMatch.forceTapWhenHittableInTestCase(self)
    ...
    func _testCardBrandChoice(isSetup: Bool = false, settings: PaymentSheetTestPlaygroundSettings) {
        ...
        XCTAssertTrue(app.textFields["Select card brand (optional)"].firstMatch.waitForExistence(timeout: 2))
