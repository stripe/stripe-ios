...
class PaymentSheetStandardUITests: PaymentSheetUITestCase {
    ...
    func testPaymentSheetFlowControllerSaveAndRemoveCard() throws {
        ...
        XCTAssertEqual(
            ...
            analyticsLog.map({ $0[string: "event"] }).filter({ !($0?.starts(with: "elements.captcha.passive") ?? false) && !($0?.contains("attestation") ?? false) }),
...
class PaymentSheetDeferredUITests: PaymentSheetUITestCase {
    ...
    func testDeferredPaymentIntent_ClientSideConfirmation() {
        ...
        XCTAssertEqual(
            ...
            // filter out async passive captcha and attestation logs
            analyticsLog.map({ $0[string: "event"] }).filter({ $0 != "luxe_image_selector_icon_from_bundle" && $0 != "luxe_image_selector_icon_downloaded" && !($0?.starts(with: "elements.captcha.passive") ?? false) && !($0?.contains("attestation") ?? false) }),
            // fraud detection telemetry should not be sent in tests, so it should report an API failure
