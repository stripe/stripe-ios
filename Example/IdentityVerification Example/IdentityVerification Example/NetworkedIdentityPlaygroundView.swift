...
final class NetworkedIdentityPlaygroundView: UIStackView {
    ...
    private let linkUISwitch = UISwitch()
    ...
    var networkedIdentityOptions: IdentityVerificationSheet.Configuration.NetworkedIdentityOptions? {
        ...
        return .init(
            ...
            // The playground supplies eligibility until its test sessions return the v8 fields.
            debugRoute: "reuse",
            debugSeedSavedDocuments: seedSwitch.isOn,
            debugUseLinkUI: linkUISwitch.isOn
        )
    }
    ...
    init() {
        ...
        addArrangedSubview(makeSwitchRow(linkUISwitch, title: "Use Link's own sign-in screens"))
...
extension NetworkedIdentityPlaygroundView {
    ...
    private func makeSeedRow() -> UIStackView {
        makeSwitchRow(seedSwitch, title: "Seed saved documents")
    }

    private func makeSwitchRow(_ toggle: UISwitch, title: String) -> UIStackView {
        let label = UILabel()
        label.text = title
