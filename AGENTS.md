...
### Assertions
Use `stpAssert` / `stpAssertionFailure` for SDK-internal sanity checks that should not assert in merchant apps. Use Swift `assert` only when you intentionally want normal Swift assertion behavior in debug builds, including merchant integrations.
