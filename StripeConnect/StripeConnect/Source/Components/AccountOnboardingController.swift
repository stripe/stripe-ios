...
public protocol AccountOnboardingControllerDelegate: AnyObject {
    /**
     ...
     the account to the next step of your flow. If an error is provided then that means onboarding exited because of an error.
     - Parameters:
     - accountOnboarding: The account onboarding controller associated with this onboarding experience
     - error: The error that occurred if an error triggered the exit.
     */
    @_documentation(visibility: public)
    func accountOnboardingDidExit(_ accountOnboarding: AccountOnboardingController, error: Error?)
...
public final class AccountOnboardingController {
    ...
    /// Sets the title for the presented onboarding experience
    @_documentation(visibility: public)
    @_spi(PrivateBetaConnect)
    ...
    init(props: Props,
        ...
        webVC = ConnectComponentWebViewController(
            ...
            self.delegate?.accountOnboardingDidExit(self, error: error)
        }
