    ...
    func sendMessageAsync(_ sender: any MessageSender) async throws {
        return try await withCheckedThrowingContinuation { continuation in
            do {
                ...
                webView.evaluateJavaScript(message, completionHandler: { data, error in
                    if let error = error {
...
private extension ConnectComponentWebViewController {
    ...
    func addMessageHandlers<InitProps: Encodable>(
        ...
        addMessageHandler(CloseWebViewMessageHandler(analyticsClient: analyticsClient, didReceiveMessage: { [weak self] payload in
    ...
    func updateColors(appearance: Appearance) {
        ...
        navAppearance.backgroundColor = appearance.colors.background ?? navAppearance.backgroundColor
