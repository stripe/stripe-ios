...
class ConnectComponentWebViewController: ConnectWebViewController {
    ...
    init<InitProps: Encodable>(
        ...
        let allowedHosts = StripeConnectConstants.allowedHosts + [self.componentManager.baseURL.host ?? ""]
        ...
        if loadContent {
            ...
            do {
                let url = try ConnectJSURLParams(component: componentType, apiClient: componentManager.apiClient, publishableKey: componentManager.publishableKeyOverride).url(baseURL: componentManager.baseURL)
