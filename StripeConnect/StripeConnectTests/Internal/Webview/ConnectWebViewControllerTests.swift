...
class ConnectWebViewControllerTests: XCTestCase {
    ...
    @MainActor
    func testAlertsConfirmationOkPressed() async throws {
        ...
        let result = try await popupConfirmation(body: message.jsonString()!, expectedAlert: returnedMessage, callConfirmCallback: true)
...
struct AlertSender: MessageSender {
    typealias Payload = ConnectWebViewController.Alert
    let name: String = "alert"
    let payload: Payload
}

struct ConfirmSender: MessageSender {
    typealias Payload = ConnectWebViewController.Alert
    let name: String = "confirm"
    let payload: Payload
}
