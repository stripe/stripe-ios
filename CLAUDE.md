...
### Filing PRs
When using the GitHub `gh` command, ALWAYS set `GH_HOST=github.com`. For example: `GH_HOST=github.com gh pr create --draft --title [...]`

- ALWAYS create PRs as **drafts** (`--draft` flag) unless the user explicitly asks for a ready-for-review PR.
- After creating a PR, ALWAYS run `/babysit-pr` to monitor CI and fix any failures automatically.

### Babysit PR Workflow
Use `/babysit-pr` to automatically monitor Bitrise CI and fix failures. Requires a Bitrise Personal Access Token. First-time setup:
1. Install direnv if you don't have it: `brew install direnv` (and add the shell hook per `direnv hook <shell>`)
2. `cp .env.example .env`
3. Get a token from https://app.bitrise.io/me/profile#/security ("Generate new token")
4. Paste it into `.env`
5. `direnv allow` (one-time, direnv will auto-load `.env` on future `cd`)

The skill pushes your branch, creates a draft PR if needed, polls Bitrise every 3 minutes, and on failure: fetches logs, analyzes the issue, attempts a fix, runs tests locally, and pushes again. Defaults to 3 max retries.
