...
class STPCardFormViewTests: XCTestCase {
    ...
    func testMarkFormErrorsLogic() {
        ...
        for shouldHandle in handledErrorsTypes {
            let error = NSError(domain: STPError.stripeDomain, code: STPErrorCode.apiError.rawValue, userInfo: [STPError.stripeErrorCodeKey: shouldHandle])
            XCTAssertTrue(cardForm.markFormErrors(for: error), "Failed to handle error for \(shouldHandle)")
        }
        ...
        for shouldNotHandle in unhandledErrorTypes {
            ...
            XCTAssertFalse(cardForm.markFormErrors(for: error), "Incorrectly handled \(shouldNotHandle ?? "nil")")
        }
    }
    ...
    func helperFunctionalTestNumber(_ cardNumber: String, shouldHandle: Bool) {
        let createPaymentIntentExpectation = self.expectation(description: "createPaymentIntentExpectation")
        var retrievedClientSecret: String? = nil
        STPTestingAPIClient.shared().createPaymentIntent(withParams: nil) { (createdPIClientSecret, error) in
        ...
        let paymentMethodParams = STPPaymentMethodParams.paramsWith(card: cardParams, billingDetails: billingDetails, metadata: nil)
        ...
        client.confirmPaymentIntent(with: paymentIntentParams) { (paymentIntent, error) in
            if let error = error {
                let cardForm = STPCardFormView()
                if shouldHandle {
                XCTAssertTrue(cardForm.markFormErrors(for: error), "Failed to handle \(error) for \(cardNumber)")
                } else {
                    XCTAssertFalse(cardForm.markFormErrors(for: error), "Incorrectly handled \(error) for \(cardNumber)")
                }
