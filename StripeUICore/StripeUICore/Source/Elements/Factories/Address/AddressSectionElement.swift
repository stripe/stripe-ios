...
@_spi(STP) public class AddressSectionElement: ContainerElement {
    ...
    public lazy var view: UIView = {
        let vStack = UIStackView(
            arrangedSubviews: [addressSection.view, sameAsCheckbox.view].compactMap { $0 }
        )
    ...
    var addressDetails: AddressDetails {
        ...
        return .init(
            name: name?.text,
            phone: phone?.phoneNumber?.string(as: .e164),
            address: address
        )
    }
    ...
    public func updateBillingSameAsShippingDefaultAddress(_ defaultAddress: AddressDetails.Address)
    {
        ...
        sameAsCheckbox.view.isHidden =
            defaultAddress == .init()
            || !countryCodes.contains(defaultAddress.country ?? "country doesnt exist")
    ...
    private func updateAddressFields(
        ...
        state =
            ...
            ? spec.makeStateElement(
                ...
                stateDict: Dictionary(
                    uniqueKeysWithValues: zip(spec.subKeys ?? [], spec.subLabels ?? [])
                ),
                theme: theme
            ) : nil
        postalCode =
            fieldOrdering.contains(.postal)
            ? spec.makePostalElement(
        ...
        let addressFields: [TextOrDropdownElement?] = fieldOrdering.reduce([]) {
            partialResult,
            fieldType in
        ...
        addressSection.elements =
            ([name] + [country] + [autoCompleteLine] + addressFields + [phone]).compactMap { $0 }
    }
...
extension AddressSectionElement: ElementDelegate {
    public func didUpdate(element: Element) {
        if !sameAsCheckbox.view.isHidden, sameAsCheckbox.isSelected,
            !displayedAddressEqualTo(address: defaults.address)
        {
