...
class TextFieldView: UIView {
    ...
    fileprivate func installConstraints() {
        hStack = UIStackView(arrangedSubviews: [
            textFieldView, errorIconView, clearButton, logoIconView,
        ])
        clearButton.setContentHuggingPriority(.required, for: .horizontal)
        clearButton.setContentCompressionResistancePriority(
            textField.contentCompressionResistancePriority(for: .horizontal) + 1,
            for: .horizontal
        )
        errorIconView.setContentHuggingPriority(.required, for: .horizontal)
        errorIconView.setContentCompressionResistancePriority(
            textField.contentCompressionResistancePriority(for: .horizontal) + 1,
            for: .horizontal
        )
        logoIconView.setContentHuggingPriority(.required, for: .horizontal)
        logoIconView.setContentCompressionResistancePriority(
            textField.contentCompressionResistancePriority(for: .horizontal) + 1,
            for: .horizontal
        )
    ...
    func updateUI(with viewModel: TextFieldElement.ViewModel) {
        ...
        let cursorOffsetFromEnd = textField.selectedTextRange.map {
            textField.offset(from: textField.endOfDocument, to: $0.end)
        }

        textField.attributedText = viewModel.attributedText
        if let cursorOffsetFromEnd = cursorOffsetFromEnd,
            let cursor = textField.position(
            ...
            error.shouldDisplay(isUserEditing: textField.isEditing)
        {
            ...
            textField.textColor =
                isUserInteractionEnabled ? viewModel.theme.colors.textFieldText : .tertiaryLabel
            errorIconView.alpha = 0
            accessibilityValue = viewModel.attributedText.string
        }
        if frame != .zero {
            // Fixes an issue on iOS 15 where setting textField properties cause it
            // to lay out from zero size.
            textField.layoutIfNeeded()
        }

        // Update logo image
        logoIconView.image = currentLogo
        // For some reason, the stackview chooses to stretch logoIconView if its image is nil
        // instead of the text field, so we hide it.
        logoIconView.isHidden = currentLogo == nil
    }
...
extension TextFieldView: UITextFieldDelegate {
    ...
    func textField(
        ...
        didReceiveAutofill =
            (text.isEmpty && range.length == 0 && range.location == 0 && string.count > 1)
        return true
    }
}
