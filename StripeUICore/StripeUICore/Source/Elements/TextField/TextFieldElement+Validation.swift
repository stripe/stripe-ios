...
@_spi(STP) extension TextFieldElement {
    @frozen public enum ValidationState {
        case valid
    ...
    @frozen public enum Error: TextFieldValidationError, Equatable {
...
extension ElementValidationState {
    ...
    init(
        from validationState: TextFieldElement.ValidationState,
        isUserEditing: Bool
    ) {
        ...
        case .invalid(let error):
            self = .invalid(
                error: error,
                shouldDisplay: error.shouldDisplay(isUserEditing: isUserEditing)
            )
        }
    }
}
