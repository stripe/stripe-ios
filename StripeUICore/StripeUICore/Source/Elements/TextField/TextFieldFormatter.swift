...
struct TextFieldFormatter {
    ...
    init?(
        ...
        guard
            format.contains(TextFieldFormatter.letterPatternCharacter)
                || format.contains(TextFieldFormatter.digitPatternCharacter)
        else {
            return nil
        }
        self.format = format
        ...
        for token in format {
            ...
            repeat {
                ...
                if token == TextFieldFormatter.digitPatternCharacter,
                    input[cursor].isNumber
                        || input[cursor] == TextFieldFormatter.redactedNumberCharacter
                {
                    consumeInput = true
