...
class AddressSectionElementTest: XCTestCase {
    ...
    func testAddressFieldsMapsSpecs() throws {
        ...
        XCTAssert(
            section.elements.first is DropdownFieldElement,
    ...
    func testAddressFieldsChangeWithCountry() {
        ...
        guard let country = section.elements.first as? DropdownFieldElement else {
            XCTFail()
            return
        }
        ...
        XCTAssertEqual(
            USTextFields.map { $0.configuration.label },
            expectedUSFields.map { $0.label }
        )
        XCTAssertEqual(
            USTextFields.map { $0.configuration.isOptional },
            expectedUSFields.map { $0.isOptional }
        )
        ...
        XCTAssertEqual(
            ZZTextFields.map { $0.configuration.label },
            expectedZZFields.map { $0.label }
        )
        XCTAssertEqual(
            ZZTextFields.map { $0.configuration.isOptional },
            expectedZZFields.map { $0.isOptional }
        )
    }
    ...
    func testCountries() {
        ...
        XCTAssertEqual(
            AddressSectionElement(title: "", countries: nil, addressSpecProvider: specProvider)
                .countryCodes,
            ["US"]
        )
        // Countries not in spec
        XCTAssertEqual(
            AddressSectionElement(title: "", countries: ["UK"], addressSpecProvider: specProvider)
                .countryCodes,
            ["UK"]
        )
        // Countries not in spec
    ...
    func test_additionalFields() {
        // Test when the field is optional and when it's required.
        for isOptional in [true, false] {
    ...
    func test_phone_country_updates_with_country_picker() {
        ...
        sut.country.select(index: 0)
        XCTAssertNotEqual(
            sut.country.selectedIndex,
            sut.phone?.countryDropdownElement.selectedIndex
        )
    }
}
