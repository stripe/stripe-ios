...
class PhoneNumberTests: XCTestCase {
    ...
    func testFormats() {
        let cases:
            [(number: String, country: String, format: PhoneNumber.Format, formattedNumber: String)] =
                [
                    (
                        number: "",
                        country: "US",
    ...
    func testFromE164_shouldDisambiguateUsingLocale() {
        ...
        XCTAssertEqual(
            PhoneNumber.fromE164(number, locale: .init(identifier: "en_US"))?.countryCode,
            "US"
        )
        XCTAssertEqual(
            PhoneNumber.fromE164(number, locale: .init(identifier: "en_CA"))?.countryCode,
            "CA"
        )
        XCTAssertEqual(
            PhoneNumber.fromE164(number, locale: .init(identifier: "es_DO"))?.countryCode,
            "DO"
        )
        XCTAssertEqual(
            PhoneNumber.fromE164(number, locale: .init(identifier: "en_PR"))?.countryCode,
            "PR"
        )
        XCTAssertEqual(
            PhoneNumber.fromE164(number, locale: .init(identifier: "en_JM"))?.countryCode,
            "JM"
        )
