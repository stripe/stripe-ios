...
def check_spi_protection(file)
  ...
  File.foreach(file) do |line|
    ...
    if line =~ /@_spi\(\w+\)\s+(public\s+|open\s+)?(class|struct|enum|protocol|typealias)\s+\w+/ && opening_braces > 0
      in_spi_protected_block = true
    elsif line =~ /(public\s+|open\s+)?(class|struct|enum|protocol|typealias)\s+\w+/ && opening_braces > 0 && !in_spi_protected_block
      in_spi_protected_block = false
