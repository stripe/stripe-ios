...
class LiquidGlassTestPlanGenerator
  ...
  def generate
    ...
    # Find all files containing @LiquidGlass
    annotated_files = find_annotated_files

    if annotated_files.empty?
      log_info "No files with @LiquidGlass annotations found"
      return
    end

    log_info "Found #{annotated_files.length} files with @LiquidGlass annotations"
  ...
  def find_annotated_files
    ...
    Find.find('.') do |path|
      ...
      # Check if file contains @LiquidGlass annotation
      if File.read(path, encoding: 'UTF-8').include?('// @LiquidGlass')
        files << path
      end
    rescue => e
  ...
  def process_annotated_file(file_path)
    ...
    lines.each_with_index do |line, index|
      if line.match?(/\/\/\s*@LiquidGlass/)
        log_info "  ↳ Found @LiquidGlass annotation at line #{index + 1}"
  ...
  def process_liquid_glass_annotation(lines, annotation_index, target_name, file_path)
    ...
    (annotation_index + 1...lines.length).each do |i|
      ...
      # If we hit something else that's not a comment or empty line, stop looking
        ...
        log_error "    ↳ @LiquidGlass not followed by class or test method: #{line}"
        exit 1
        break
      end
    end
  ...
  def log_results
    if File.exist?(@test_plan_file)
      ...
      if total_tests > 0
        ...
        log_info "No @LiquidGlass tests found."
      end
    else
