...
class AppStoreConnectAPI
    ...
    def buildRuns(ci_product_id)
      self.class.get("/ciProducts/#{ci_product_id}/buildRuns", {
                       headers: @headers,
                       query: {
    ...
    def actions(ci_buildrun_id)
        self.class.get("/ciBuildRuns/#{ci_buildrun_id}/actions", {
                         headers: @headers,
                         query: {
                            # 'fields[ciBuildActions]' => 'artifacts',
                         }
                       })
      end

    def artifacts(ci_buildaction_id)
        self.class.get("/ciBuildActions/#{ci_buildaction_id}/artifacts", {
                         headers: @headers,
                         query: {
                         }
                       })
  ...
  build_run = build_runs.filter { |build_run| build_run['attributes']['sourceCommit']['commitSha'] == build_commit && build_run['attributes']['completionStatus'] == 'SUCCEEDED' }.first
  ...
  build_action = actions.find { |action| action['attributes']['name'] == 'Build - iOS' && action['attributes']['completionStatus'] == 'SUCCEEDED' }
