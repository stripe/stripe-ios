...
def run_sourcekitten
  ...
  schemes.each do |s|
    ...
    `sourcekitten doc --module-name #{s[:module]} -- build -configuration Release CONFIGURATION_BUILD_DIR="#{$TEMP_BUILD_DIR}" -workspace Stripe.xcworkspace -destination 'generic/platform=iOS' -scheme #{s[:scheme]} > #{output_file}`
