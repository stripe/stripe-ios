...
class APIRequest<ResponseType: STPAPIResponseDecodable>: NSObject {
  ...
  class func parseResponse<ResponseType: STPAPIResponseDecodable>(
    ...
    //        let objectString = jsonDictionary?[JSONKeyObject] as? String
    //
    //        var deserializerClass: AnyClass? = nil
    //        if deserializers.count == 1 {
    //            // Some deserializers don't conform to STPInternalAPIResponseDecodable
    //            deserializerClass = type(of: deserializers.first!)
    //        } else if let objectString = objectString {
    //
    //            for deserializer in deserializers {
    //                if deserializer.responds(to: #selector(STPAPIResponseDecodable.stripeObject)) && ((deserializer as? STPInternalAPIResponseDecodable)?.stripeObject() == objectString) {
    //                    // Found matching deserializer
    //                    deserializerClass = type(of: deserializer)
    //                }
    //            }
    //        }

    //        weak var responseObject: STPAPIResponseDecodable? = nil
    //        if let deserializerClass = deserializerClass {
    //            // Generate response object
    //            responseObject = deserializerClass.decodedObject(fromAPIResponse: jsonDictionary)
    //        }
    //
    //        if responseObject == nil {
    //            // Failed to parse response
    //            let parsedError = NSError.stp_error(fromStripeResponse: jsonDictionary)
    //
    ...
    //        }
    //
    //        return safeCompletion(responseObject as! ResponseType, nil)
  }
