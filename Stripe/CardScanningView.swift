...
fileprivate class CardScanningEasilyTappableButton: UIButton {
...
class CardScanningView: UIView, STPCardScannerDelegate {
  ...
  func cardScanner(_ scanner: STPCardScanner, didFinishWith cardParams: STPPaymentMethodCardParams?, error: Error?) {
    if error != nil {
  ...
  private lazy var closeButton : UIButton = {
    ...
    button.accessibilityLabel = STPLocalizedString("Close card scanner", "Accessibility label for the button to close the card scanner.")
  ...
  private func setupBlurView() {
    ...
    cardOuterBlurView.addConstraints([
        vibrancyEffectView.bottomAnchor.constraint(equalTo: cardOuterBlurView.bottomAnchor, constant: 0),
        vibrancyEffectView.leftAnchor.constraint(equalTo: cardOuterBlurView.leftAnchor, constant: 0),
        vibrancyEffectView.rightAnchor.constraint(equalTo: cardOuterBlurView.rightAnchor, constant: 0),
        vibrancyEffectView.topAnchor.constraint(equalTo: cardOuterBlurView.topAnchor, constant: 0),
    ])

    vibrancyEffectView.addConstraints([
      closeButton.rightAnchor.constraint(equalTo: vibrancyEffectView.rightAnchor, constant: -8),
      closeButton.topAnchor.constraint(equalTo: vibrancyEffectView.topAnchor, constant: 8),

      instructionsLabel.leftAnchor.constraint(equalTo: vibrancyEffectView.leftAnchor, constant: 0),
      instructionsLabel.rightAnchor.constraint(equalTo: vibrancyEffectView.rightAnchor, constant: 0),
    ])
  }
  ...
  override init(frame: CGRect) {
    ...
    cameraView.accessibilityLabel = STPLocalizedString("Point the camera at your card.", "Accessibility instructions for card scanning.")
    ...
    self.addConstraints(
      [
        ...
        errorLabel.rightAnchor.constraint(equalTo: cardOutlineView.rightAnchor, constant: -8),

        cardOutlineView.heightAnchor.constraint(
          equalTo: cardOutlineView.widthAnchor, multiplier: CardScanningView.cardSizeRatio),

        cardOutlineView.topAnchor.constraint(equalTo: self.topAnchor, constant: CardScanningView.cardInset),
        cardOutlineView.leftAnchor.constraint(equalTo: self.leftAnchor, constant: CardScanningView.cardInset),
        cardOutlineView.rightAnchor.constraint(equalTo: self.rightAnchor, constant: -CardScanningView.cardInset),
        cardOutlineView.bottomAnchor.constraint(equalTo: self.bottomAnchor, constant: -CardScanningView.cardInset),
      ])
  }

  override func layoutSubviews() {
    super.layoutSubviews()
    let cornerRadius = (self.bounds.size.width - (CardScanningView.cardInset * 2)) * CardScanningView.cardCornerRadius
    cardOutlineView.layer.cornerRadius = cornerRadius
    
    let outerPath = UIBezierPath(roundedRect: CGRect(x: 0, y: 0, width: self.bounds.size.width, height: self.bounds.size.height),
