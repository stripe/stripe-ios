...
class STPFloatingPlaceholderTextField: UITextField {
    ...
    var defaultPlaceholderColor: UIColor = STPFloatingPlaceholderTextField.LayoutConstants.defaultPlaceholderColor
    var floatingPlaceholderColor: UIColor = STPFloatingPlaceholderTextField.LayoutConstants.floatingPlaceholderColor
    ...
    func floatingPlaceholderHeight() -> CGFloat {
        let placeholderLabelHeight = placeholderLabel.textRect(forBounds: CGRect(x: 0, y: 0, width: CGFloat.greatestFiniteMagnitude, height:  CGFloat.greatestFiniteMagnitude), limitedToNumberOfLines: 1).height
        return placeholderLabelHeight * STPFloatingPlaceholderTextField.LayoutConstants.floatingPlaceholderScale
    }
    ...
    func contentPadding() -> UIEdgeInsets {
        ...
        return UIEdgeInsets(top: vMargin, left: isRTL ? rightMargin : leftMargin, bottom: vMargin, right: isRTL ? leftMargin: rightMargin)
    }
    ...
    func layoutPlaceholder(animated: Bool) {
        ...
        placeholderFrame.size.width = min(placeholderFrame.size.width, placeholderLabel.textRect(forBounds: placeholderFrame, limitedToNumberOfLines: 1).width)
        ...
        if minimized {
            ...
            if traitCollection.layoutDirection == .rightToLeft {
                ...
                placeholderFrame.origin.x = placeholderFrame.origin.x + (1-scale)*placeholderFrame.width
            }
            placeholderFrame.size.width = placeholderFrame.width * scale // scaling the width here leads to a clean up and down animation
            placeholderFrame.size.height = placeholderLabel.textRect(forBounds: placeholderFrame, limitedToNumberOfLines: 1).height * scale
...
extension STPFloatingPlaceholderTextField {
    ...
    @objc public override func leftViewRect(forBounds bounds: CGRect) -> CGRect {
        ...
        leftViewRect.origin.x = leftViewRect.origin.x +  STPFloatingPlaceholderTextField.LayoutConstants.horizontalMargin
        return leftViewRect
    }
    ...
    @objc public override func rightViewRect(forBounds bounds: CGRect) -> CGRect {
        ...
        rightViewRect.origin.x = rightViewRect.origin.x - STPFloatingPlaceholderTextField.LayoutConstants.horizontalMargin
    ...
    @objc public override var intrinsicContentSize: CGSize {
        get {
            let height = UIFontMetrics.default.scaledValue(for: STPFloatingPlaceholderTextField.LayoutConstants.defaultHeight)
            let contentPadding = self.contentPadding()
            return CGSize(width: placeholderLabel.intrinsicContentSize.width + contentPadding.left + contentPadding.right , height: height)
        }
    }
