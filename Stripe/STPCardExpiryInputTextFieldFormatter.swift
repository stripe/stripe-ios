...
class STPCardExpiryInputTextFieldFormatter: STPInputTextFieldFormatter {
    ...
    override func isAllowedInput(_ input: String, to string: String, at range: NSRange) -> Bool {
        ...
        let proposed = string.replacingCharacters(in: range, with: input).stp_stringByRemovingCharacters(from: .whitespaces)
        var proposedComponents = proposed.split(separator: "/").map( { String($0) }).filter({ !$0.isEmpty })
        if let firstComponent = proposedComponents.first,
           firstComponent == proposed {
            // we don't have a separator, so go by index
            proposedComponents = [proposed.stp_safeSubstring(to: 2), proposed.stp_safeSubstring(from: 2)].filter({ !$0.isEmpty })
        }
        
        if proposedComponents.count > 2 {
            return false
        } else if proposedComponents.first(where: { !STPNumericStringValidator.isStringNumeric(String($0)) }) != nil {
            return false
