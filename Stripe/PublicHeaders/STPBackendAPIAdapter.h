...
/**
 *  Adds a card token to a customer. On your backend, retrieve the Stripe Customer associated with your logged-in user. Then, call the Create Card method on that customer as described at https://stripe.com/docs/api#create_card (for an example Ruby implementation of this API, see https://github.com/stripe/example-ios-backend/blob/master/web.rb#L60 ). Then, return their cards and selected card as described in the `retrieveCards` documentation and call `completion` with them.
 *
 *  @param token      the Stripe token representing the user's newly-added credit card.
 *  @param completion call this callback when you're done adding the token to the customer on your server. For example, completion(selectedCard, cards, nil) (if your call succeeds) or completion(nil, nil, error) if an error is returned.
 */
- (void)addToken:(nonnull STPToken *)token completion:(nonnull STPCardCompletionBlock)completion;

/**
 *  Change a customer's default_source to be the provided card. On your backend, retrieve the Stripe Customer associated with your logged-in user. Then, call the Customer Update method as described at https://stripe.com/docs/api#update_customer , specifying default_source to be the provided card (for an example Ruby implementation of this API, see https://github.com/stripe/example-ios-backend/blob/master/web.rb#L82 ). Then, return their cards and selected card as described in the `retrieveCards` documentation and call `completion` with them.
 *
 *  @param card       The newly-selected default source for the user.
 *  @param completion call this callback when you're done selecting the new default source for the customer on your server. For example, completion(selectedCard, cards, nil) (if your call succeeds) or completion(nil, nil, error) if an error is returned.
 */
