...
class STPPaymentIntentFunctionalTest: XCTestCase {
    ...
    func testCreatePaymentIntentWithInvalidCurrency() {
        ...
        STPTestingAPIClient.shared().createPaymentIntent(withParams: [
            ...
            XCTAssertTrue((error as NSError?)?.userInfo[STPError.errorMessageKey()]?.hasPrefix("Error creating PaymentIntent: The currency provided (usd) is invalid. Payments with bancontact support the following currencies: eur."))
            expectation.fulfill()
        }
        waitForExpectations(timeout: TestConstants.stpTestingNetworkRequestTimeout, handler: nil)
    }
    ...
    func testRetrievePreviousCreatedPaymentIntent() {
        ...
        client.retrievePaymentIntent(
            ...
            XCTAssertNotNil(Int(paymentIntent ?? 0))
            XCTAssertEqual(paymentIntent?.stripeId, "pi_1GGCGfFY0qyl6XeWbSAsh2hn")
            XCTAssertEqual(paymentIntent?.amount ?? 0, 100)
            XCTAssertEqual(paymentIntent?.currency, "usd")
            XCTAssertFalse(paymentIntent?.livemode)
            XCTAssertNil(paymentIntent?.sourceId ?? 0)
            XCTAssertNil(paymentIntent?.paymentMethodId ?? 0)
            XCTAssertEqual(paymentIntent?.status ?? 0, Int(STPPaymentIntentStatusCanceled))
            XCTAssertEqual(paymentIntent?.setupFutureUsage ?? 0, Int(STPPaymentIntentSetupFutureUsageNone))
            //#pragma clang diagnostic push
            //#pragma clang diagnostic ignored "-Wdeprecated"
            XCTAssertNil(paymentIntent?.nextSourceAction ?? 0)
            //#pragma clang diagnostic pop
            XCTAssertNil(paymentIntent?.nextAction ?? 0)
    ...
    func testRetrieveWithWrongSecret() {
        ...
        client.retrievePaymentIntent(
            ...
            XCTAssertNil(Int(paymentIntent ?? 0))

            XCTAssertNotNil(error)
            XCTAssertEqual((error as NSError?)?.domain, STPError.stripeDomain())
            XCTAssertEqual((error as NSError?)?.code ?? 0, Int(STPInvalidRequestError))
            XCTAssertEqual(
                (error as NSError?)?.userInfo[STPError.errorParameterKey()],
                "clientSecret")
    ...
    func testConfirmCanceledPaymentIntentFails() {
        ...
        client.confirmPaymentIntent(
            ...
            XCTAssertEqual((error as NSError?)?.code ?? 0, Int(STPInvalidRequestError))
            XCTAssertTrue(
                (error as NSError?)?.userInfo[STPError.errorMessageKey()]?.hasPrefix("This PaymentIntent's source could not be updated because it has a status of canceled. You may only update the source of a PaymentIntent with one of the following statuses: requires_payment_method, requires_confirmation, requires_action."),
                "Expected error message to complain about status being canceled. Actual msg: `%@`",
                (error as NSError?)?.userInfo[STPError.errorMessageKey()])
    ...
    func testConfirmPaymentIntentWith3DSCardSucceeds() {
        ...
        client.confirmPaymentIntent(
            ...
            XCTAssertEqual(paymentIntent?.status ?? 0, Int(STPPaymentIntentStatusRequiresAction))

            // STPRedirectContext is relying on receiving returnURL
            XCTAssertNotNil(paymentIntent?.nextAction.redirectToURL.returnURL ?? 0)
            XCTAssertEqual(
            ...
            //#pragma clang diagnostic push
            //#pragma clang diagnostic ignored "-Wdeprecated"
            XCTAssertNotNil(paymentIntent?.nextSourceAction.authorizeWithURL.returnURL ?? 0)
            XCTAssertEqual(
    ...
    func testConfirmPaymentIntentWith3DSCardPaymentMethodSucceeds() {
        ...
        client.confirmPaymentIntent(
            ...
            XCTAssertEqual(paymentIntent?.stripeId, params.stripeId)
            XCTAssertFalse(paymentIntent?.livemode)
            XCTAssertNotNil(paymentIntent?.paymentMethodId ?? 0)
    ...
    func testConfirmCardWithoutNetworkParam() {
        ...
        client.confirmPaymentIntent(
            ...
            XCTAssertEqual(paymentIntent?.status ?? 0, Int(STPPaymentIntentStatusSucceeded))
    ...
    func testConfirmAUBECSDebitPaymentIntent() {
        ...
        client.confirmPaymentIntent(
            ...
            XCTAssertEqual(paymentIntent?.status ?? 0, Int(STPPaymentIntentStatusProcessing))
    ...
    func testConfirmPaymentIntentWithOXXO() {
        ...
        client.confirmPaymentIntent(
            ...
            XCTAssertNotNil(Int(oxxoDisplayDetails?["expires_after"] ?? 0))
            XCTAssertNotNil(Int(oxxoDisplayDetails?["number"] ?? 0))
            XCTAssertEqual(paymentIntent?.status ?? 0, Int(STPPaymentIntentStatusRequiresAction))
            expectation.fulfill()
        }
    ...
    func testConfirmAlipayPaymentIntent() {
        ...
        client.confirmPaymentIntent(
            ...
            XCTAssertEqual(paymentIntent?.status ?? 0, Int(STPPaymentIntentStatusRequiresAction))
            XCTAssertEqual(paymentIntent?.nextAction.type ?? 0, Int(STPIntentActionTypeAlipayHandleRedirect))
            XCTAssertNotNil(paymentIntent?.nextAction ?? 0)
    ...
    func testConfirmPaymentIntentWithBLIK() {
        ...
        client.confirmPaymentIntent(
            ...
            XCTAssertEqual(paymentIntent?.status ?? 0, Int(STPPaymentIntentStatusRequiresAction))
            XCTAssertEqual(paymentIntent?.nextAction.type ?? 0, Int(STPIntentActionTypeBLIKAuthorize))
    ...
    func testConfirmPaymentIntentWithAffirm() {
        ...
        client.confirmPaymentIntent(
            ...
            XCTAssertEqual(paymentIntent?.status ?? 0, Int(STPPaymentIntentStatusRequiresAction))
            XCTAssertEqual(paymentIntent?.nextAction.type ?? 0, Int(STPIntentActionTypeRedirectToURL))
            expectation.fulfill()
        }
    ...
    // MARK: - Test Objective-C setupFutureUsage

    func testObjectiveCSetupFutureUsage() {
        let params = STPPaymentIntentParams()
        params.setupFutureUsage = NSNumber(value: STPPaymentIntentSetupFutureUsageOnSession)
        XCTAssertEqual(params.setupFutureUsageRawString, "on_session")
    }
    ...
    func cardSourceParams() -> STPSourceParams? {
        ...
        card.expMonth = 7
        card.expYear = Calendar.current.component(.year, from: Date()) + 5
        card.currency = "usd"
