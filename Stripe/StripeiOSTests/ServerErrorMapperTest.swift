...
class ServerErrorMapperTest: XCTestCase {
    ...
    func testFromInvalidCustEphKey() {
        ...
        XCTAssertTrue(
            ...
            )!.hasPrefix("Invalid customer ephemeral key secret.")
        )
    }
    ...
    func testFromNoSuchPaymentIntent() {
        ...
        XCTAssertTrue(
            ServerErrorMapper.mobileErrorMessage(
                from: serverErrorMessage,
                httpResponse: HTTPURLResponse()
            )!.hasPrefix("No matching PaymentIntent could")
        )
    }
    ...
    func testFromNoSuchSetupIntent() {
        ...
        XCTAssertTrue(
            ServerErrorMapper.mobileErrorMessage(
                from: serverErrorMessage,
                httpResponse: HTTPURLResponse()
            )!.hasPrefix("No matching SetupIntent could")
        )
    }

    func testFromUnknownErrorMessage() {
        let serverErrorMessage =
            "This error message is not known to ServerErrorMapper, should return nil."
