...
class STPCardParamsTest: XCTestCase {
    ...
    func testCopyWithZone() {
        ...
        let copiedCardParams = cardParams.copy()
        ...
        for property in STPTestUtils.propertyNamesOf(cardParams) ?? [] {
            if notEqualProperties.contains(property) {
                XCTAssertNotEqualObjects(
                    cardParams.value(forKey: property),
                    copiedCardParams?.value(forKey: property),
                    "%@",
                    property)
            } else {
                XCTAssertEqual(
