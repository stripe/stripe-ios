...
class STPPaymentMethodUSBankAccountParamsTest: XCTestCase {
    ...
    func testCreateUSBankAccountPaymentMethod_checking_individual() {
        ...
        apiClient.createPaymentMethod(with: params) {
    ...
    func testCreateUSBankAccountPaymentMethod_savings_company() {
        ...
        apiClient.createPaymentMethod(with: params) {
            ...
            XCTAssertEqual(
                paymentMethod?.usBankAccount?.accountHolderType,
                .company,
                "`accountHolderType` should be company"
            )
            XCTAssertEqual(
                paymentMethod?.usBankAccount?.accountType,
                .savings,
                "`accountType` should be savings"
            )
            XCTAssertEqual(paymentMethod?.usBankAccount?.last4, "6789", "`last4` should be 6789")
    ...
    func testEncodingWithLinkAccountSessionID() {
        ...
        XCTAssertEqual(
            (encoded["us_bank_account"] as? [AnyHashable: Any])?["link_account_session"] as? String,
            "las_test"
        )
    }
