...
extension PaymentSheet {
    public enum PaymentMethodType: Equatable, Hashable {
        ...
        func makeImage(
            forDarkBackground: Bool = false,
            updateHandler: DownloadManager.UpdateImageHandler?
        ) -> UIImage {
                ...
                var imageUrl = URL(string: selectorIcon.lightThemePng)
            {
                ...
                if PaymentSheet.PaymentMethodType.shouldLogAnalytic(paymentMethod: self) {
                    STPAnalyticsClient.sharedClient.logImageSelectorIconDownloadedIfNeeded(
                        paymentMethod: self
                    )
                }
            ...
            if let stpPaymentMethodType = stpPaymentMethodType {
                if PaymentSheet.PaymentMethodType.shouldLogAnalytic(paymentMethod: self) {
                    STPAnalyticsClient.sharedClient.logImageSelectorIconFromBundleIfNeeded(
                        paymentMethod: self
                    )
                }
            ...
            if PaymentSheet.PaymentMethodType.shouldLogAnalytic(paymentMethod: self) {
                STPAnalyticsClient.sharedClient.logImageSelectorIconNotFoundIfNeeded(
                    paymentMethod: self
                )
            }
        ...
        static func recommendedPaymentMethodTypes(from intent: Intent) -> [PaymentMethodType] {
            ...
            case .paymentIntent(let paymentIntent):
                guard
                    let paymentMethodTypeStrings =
                        paymentIntent.allResponseFields["payment_method_types"] as? [String]
                else {
                    return []
                }
            ...
            case .setupIntent(let setupIntent):
                guard
                    let paymentMethodTypeStrings =
                        setupIntent.allResponseFields["payment_method_types"] as? [String]
                else {
                    return []
                }
                let paymentTypesString =
        ...
        static func filteredPaymentMethodTypes(
            from intent: Intent,
...
extension Array where Element == PaymentSheet.PaymentMethodType {
    ...
    func symmetricDifference(_ other: Array) -> Array
    where Element == PaymentSheet.PaymentMethodType {
        let set1 = Set(self)
        let set2 = Set(other)
