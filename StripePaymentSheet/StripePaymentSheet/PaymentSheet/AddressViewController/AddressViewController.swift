...
public class AddressViewController: UIViewController {
    ...
    lazy var scrollViewBottomConstraint: NSLayoutConstraint = {
        return scrollView.frameLayoutGuide.bottomAnchor.constraint(
    ...
    lazy var formElement: FormElement = {
        let formElement = FormElement(
            elements: [addressSection, checkboxElement],
    ...
    override public func viewDidLoad() {
        ...
        STPAnalyticsClient.sharedClient.addClass(
            toProductUsageIfNecessary: AddressViewController.self
        )
    ...
    override public func viewDidAppear(_ animated: Bool) {
        ...
        if !didLogAddressShow {
            STPAnalyticsClient.sharedClient.logAddressShow(
                defaultCountryCode: addressSection?.selectedCountryCode ?? "",
...
extension AddressViewController {
    ...
    private func expandAddressSectionIfNeeded() {
        ...
        if let addressSection = addressSection, addressSection.collectionMode == .autoCompletable,
            !configuration.autocompleteCountries.map({ $0.lowercased() }).contains(
                addressSection.selectedCountryCode.lowercased()
            )
        {
    ...
    private func initAddressSection() {
        ...
        addressSection = AddressSectionElement(
            ...
            collectionMode: configuration.defaultValues.address != .init()
                ? .all : .autoCompletable,
    ...
    private func loadUI() {
        ...
        NSLayoutConstraint.activate([
            ...
            scrollView.frameLayoutGuide.widthAnchor.constraint(
                equalTo: scrollView.contentLayoutGuide.widthAnchor
            ),
            ...
            button.leadingAnchor.constraint(
                equalTo: scrollView.contentLayoutGuide.leadingAnchor,
                constant: PaymentSheetUI.defaultSheetMargins.leading
            ),
    ...
    private func logAddressCompleted() {
        ...
        if let selectedAddress = addressDetails?.address,
            let autoCompleteAddress = selectedAutoCompleteResult
        {
            editDistance = PaymentSheet.Address(from: selectedAddress).editDistance(
...
@_spi(STP) extension AddressViewController: ElementDelegate {
    @_spi(STP) public func didUpdate(element: Element) {
        guard let addressSection = addressSection else {
            assertionFailure()
            return
        }
...
extension AddressViewController: AutoCompleteViewControllerDelegate {
    ...
    func didSelectAddress(_ address: PaymentSheet.Address?) {
        ...
        let autocompleteCountryIndex = addressSection.countryCodes.firstIndex(where: {
            $0 == address.country
        })
...
extension AddressViewController: UIGestureRecognizerDelegate {
    public func gestureRecognizer(
        ...
        // Without this, the UIControl (e.g. button) doesn't get the touch
...
extension AddressSectionElement.AddressDetails {
    init(
        from addressDetails: AddressViewController.Configuration.DefaultAddressDetails
    ) {
        self.init(
...
extension AddressSectionElement.AdditionalFields {
    init(
        from additionalFields: AddressViewController.Configuration.AdditionalFields
    ) {
        func config(
            from fieldConfiguration: AddressViewController.Configuration.AdditionalFields
                .FieldConfiguration
        ) -> FieldConfiguration {
