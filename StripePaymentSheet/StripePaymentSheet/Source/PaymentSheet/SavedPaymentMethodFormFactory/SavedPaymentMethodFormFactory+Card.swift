...
extension SavedPaymentMethodFormFactory {
    func makeCard(configuration: UpdatePaymentMethodViewController.Configuration) -> PaymentMethodElement {
        ...
        let cardBrandSelectorWrapper: PaymentMethodElementWrapper<CardBrandSelectorElement>? = {
            ...
            let disallowedCardBrands = Set(cardBrands.filter { !configuration.cardBrandFilter.isAccepted(cardBrand: $0) })

            let selectorElement = CardBrandSelectorElement()
            selectorElement.update(brands: cardBrands, disallowedBrands: disallowedCardBrands)
            // For the saved PM flow, always show sublabel when no brand is selected
            selectorElement.isPanFieldEditing = true

            // Pre-select current card brand
            if let currentCardBrand = configuration.paymentMethod.card?.preferredDisplayBrand {
                selectorElement.select(brand: currentCardBrand)
            }

            // Handler when user selects different card brand
            let wrappedElement = PaymentMethodElementWrapper<CardBrandSelectorElement>(selectorElement) { element, params in
                if let selectedBrand = element.selectedBrand {
                    let preferredNetworkAPIValue = STPCardBrandUtilities.apiValue(from: selectedBrand)
        ...
        let panElement: TextFieldElement = {
            let panElementConfig = TextFieldElement.LastFourConfiguration(lastFour: configuration.paymentMethod.card?.last4 ?? "",
                                                                          editConfiguration: cardBrandSelectorWrapper != nil ? .readOnlyWithoutDisabledAppearance : .readOnly,
        ...
        let cardSection: SectionElement = {
            let allSubElements: [Element?] = [
                panElement,
                SectionElement.HiddenElement(cardBrandSelectorWrapper),
