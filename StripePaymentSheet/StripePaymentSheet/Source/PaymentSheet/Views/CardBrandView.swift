...
final class CardBrandView: UIView {
    ...
    @_spi(STP) public func setCardBrand(_ brand: STPCardBrand) {
        setCardBrand(brand, animated: false)
    }
    ...
    func setCardBrand(_ newBrand: STPCardBrand, animated: Bool) {
        let newImage = image(for: newBrand)

        // Image has changed
        let canAnimateTransition = imageView.image != newImage
    ...
    private func image(for brand: STPCardBrand) -> UIImage {
        if showCVC {
            return PaymentSheetImageLibrary.cvcImage(for: brand)
        }
