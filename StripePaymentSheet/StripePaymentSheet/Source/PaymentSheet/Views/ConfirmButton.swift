...
class ConfirmButton: UIControl {
    ...
    init(
        ...
        update(status: status, callToAction: callToAction, animated: false)
    ...
    @objc private func didBecomeActive() {
        update(status: self.status, callToAction: self.callToAction, animated: false)
    }
    ...
    private func animateSuccess() {
        ...
        setNeedsLayout()
        UIView.animate(
            withDuration: Self.spinnerMoveToCenterAnimationDuration, delay: 0, options: .curveEaseOut
        ) {
            self.layoutIfNeeded()
        } completion: { (_) in
        }
