...
fileprivate extension PaymentSheet.FlowController {
    static func _mockFlowController() -> PaymentSheet.FlowController {
        ...
        let customerProvider = CustomerProvider.make(mode: .deferredIntent(intentConfig), configuration: psConfig)
        let loadResult = PaymentSheetLoader.LoadResult(intent: intent, elementsSession: elementsSession, savedPaymentMethods: [], paymentMethodTypes: [], customerProvider: customerProvider)
