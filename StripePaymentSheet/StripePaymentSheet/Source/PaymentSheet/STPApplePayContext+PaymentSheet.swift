...
private class ApplePayContextClosureDelegate: NSObject, ApplePayContextDelegate {
    ...
    // When true, sync the billing address to the session as it changes in the sheet, and restore it on cancel.
    let syncsBillingAddressForTax: Bool
    ...
    init(
        ...
        syncsBillingAddressForTax: Bool = false,
        ...
        self.syncsBillingAddressForTax = syncsBillingAddressForTax
    ...
    private func handleCheckoutSessionApplePay(
        ...
        if intent.collectsTaxRegionFromBillingAddress,
    ...
    private func restoreBillingAddressAfterCancel() {
        guard syncsBillingAddressForTax, case .checkout(let checkout) = intent else {
            return
        }
...
extension STPApplePayContext {
    ...
    static func create(
        ...
        let syncsBillingAddressForTax = intent.collectsTaxRegionFromBillingAddress
        if syncsBillingAddressForTax, case .checkout(let checkout) = intent {
            ...
            paymentMethodUpdateHandler = { pkPaymentMethod, completion in
                Task { @MainActor in
                    ...
                    let items = STPApplePayContext.checkoutPaymentSummaryItems(
                        checkout: checkout,
                        label: label,
                        currency: currency
                    )
        ...
        let delegate = ApplePayContextClosureDelegate(
            ...
            syncsBillingAddressForTax: syncsBillingAddressForTax,
    ...
    static func createPaymentRequest(
        ...
        if let paymentSummaryItems = applePay.paymentSummaryItems {
            ...
            paymentRequest.paymentSummaryItems = STPApplePayContext.checkoutPaymentSummaryItems(
                checkout: checkout,
                label: label,
...
private func makeRequiredBillingDetails(
    ...
    // ...also when the session needs it for tax
