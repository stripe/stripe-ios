...
enum ExpressCheckoutElementUtilities {
    ...
    static func availablePaymentMethods(
            ...
            case .applePay:
                if let applePayConfiguration = configuration.applePayConfiguration,
                   ...
                   StripeAPI.deviceSupportsApplePay() {
                    availablePaymentMethods.append(paymentMethod.rawValue)
                }
            case .link:
        ...
        var reorderedPaymentMethodTypes: [String] = []

        // 1. Add each PM in paymentMethodOrder first
        for paymentMethod in paymentMethodOrder {
            guard
                // Ignore the PM if it's not in availablePaymentMethods
                let index = availablePaymentMethods.firstIndex(where: {
                    $0.caseInsensitiveCompare(paymentMethod) == .orderedSame
                }),
                // Ignore duplicate PMs
                !reorderedPaymentMethodTypes.caseInsensitiveContains(paymentMethod)
            else {
                continue
            }
            reorderedPaymentMethodTypes.append(availablePaymentMethods.remove(at: index))
        }
        reorderedPaymentMethodTypes.append(contentsOf: availablePaymentMethods)
