...
@_spi(LinkOnly) public class LinkBankPaymentController {
    ...
    // TODO(vardges): do we need the paymentOption?
    ...
    // TODO(vardges): do we need the billing details?
    ...
    @_spi(LinkOnly) public func present(from presentingViewController: UIViewController) async throws {
        let linkController: InstantDebitsOnlyViewController = try await withCheckedThrowingContinuation { [self] continuation in
            PaymentSheet.load(mode: mode, configuration: configuration) { result in
                ...
                case .success(let intent, _, let isLinkEnabled):
                    ...
                    self.intent = intent
                    let linkController = InstantDebitsOnlyViewController(apiClient: self.configuration.apiClient)
...
@available(iOSApplicationExtension, unavailable)
@available(macCatalystApplicationExtension, unavailable)
extension LinkBankPaymentController: PayWithLinkViewControllerDelegate {
