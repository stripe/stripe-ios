...
extension EmbeddedPaymentElement: EmbeddedPaymentMethodsViewDelegate {
    ...
    func presentSavedPaymentMethods(selectedSavedPaymentMethod: STPPaymentMethod?) {
        // Special case, only 1 card remaining but is co-branded (or defaultSPMNavigation), skip showing the list and show update view controller
        if savedPaymentMethods.count == 1,
           let paymentMethod = savedPaymentMethods.first,
           paymentMethod.isCoBrandedCard,
           elementsSession.isCardBrandChoiceEligible || configuration.defaultSPMNavigation {
