...
final class PaymentSheetLoader {
    ...
    static func load(
        ...
        isUpdate: Bool = false,
        completion: @MainActor @escaping (Result<(LoadResult, ConfirmationChallenge), Error>) -> Void
    ) {
        Task { @MainActor in
            do {
    ...
    static func filterSavedPaymentMethods(
        ...
        } else if case let .checkout(context) = intent,
                  let customerPaymentMethods = context.session?.customer?.paymentMethods {
