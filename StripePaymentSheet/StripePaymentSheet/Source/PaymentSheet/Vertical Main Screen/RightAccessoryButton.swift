...
extension RowButton.RightAccessoryButton {
    ...
    ///   - isFlatCheckmarkOrChevronStyle: True if embedded and style of `flatWithCheckmark` or `flatWithDisclosure`
    ...
    static func getAccessoryButtonType(savedPaymentMethodsCount: Int,
                                       ...
                                       isFlatCheckmarkOrDisclosureStyle: Bool = false) -> AccessoryType? {
        ...
        if savedPaymentMethodsCount > 1 {
            return isFlatCheckmarkOrDisclosureStyle ? .viewMore : .viewMoreChevron
        }
