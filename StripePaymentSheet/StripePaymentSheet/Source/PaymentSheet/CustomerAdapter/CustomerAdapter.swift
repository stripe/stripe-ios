...
@_spi(PrivateBetaCustomerSheet) public protocol CustomerAdapter {
    ...
    func setSelectedPaymentOption(paymentOption: CustomerSheet.PaymentOption?) async throws

    /// Retrieve the last selected payment method for the customer.
    /// If you are implementing your own <CustomerAdapter>:
    /// Return a CustomerSheet.PaymentOption for the customer's default selected payment method.
    /// If no default payment method is selected, return nil.
    func fetchSelectedPaymentOption() async throws -> CustomerSheet.PaymentOption?
...
@_spi(PrivateBetaCustomerSheet) open class StripeCustomerAdapter: CustomerAdapter {
    ...
    open func setSelectedPaymentOption(paymentOption: CustomerSheet.PaymentOption?) async throws {
        let customerEphemeralKey = try await customerEphemeralKey

        CustomerSheet.PaymentOption.setDefaultPaymentMethod(paymentOption, forCustomer: customerEphemeralKey.id)
    }

    open func fetchSelectedPaymentOption() async throws -> CustomerSheet.PaymentOption? {
        let customerEphemeralKey = try await customerEphemeralKey

        return CustomerSheet.PaymentOption.defaultPaymentMethod(for: customerEphemeralKey.id)
    }
