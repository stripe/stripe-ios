...
final class LinkPaymentMethodFormElement: Element {
    ...
    var params: Params? {
        ...
        // with a full address because extra fields won't match Link's expected billing details.
        let billingDetails = STPPaymentMethodBillingDetails()
    ...
    private lazy var cardBrandSelector: PaymentMethodElementWrapper<CardBrandChoiceElement>? = {
        ...
        if let selectedBrand = paymentMethod.cardDetails?.cardBrand {
            cardBrandChoiceElement.select(selectedBrand)
        }
    ...
    private lazy var billingAddressSection: AddressSectionElement? = {
        ...
        return AddressSectionElement(
            ...
            defaultFieldsToCollect: configuration.billingDetailsCollectionConfiguration.address == .full
                ? .all()
                : .country,
