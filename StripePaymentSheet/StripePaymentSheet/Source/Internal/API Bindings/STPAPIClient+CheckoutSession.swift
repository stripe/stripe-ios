...
extension STPAPIClient {
    ...
    ///   - expectedAmount: The expected amount for validation, from `Checkout.Session.expectedAmountForConfirm()`.
    ///     The server validates this against `total_summary.due`, so it should be sent even when `0`.
