...
final class AddPaymentMethodViewControllerSnapshotTests: STPSnapshotTestCase {
    ...
    func test_with_previous_customer_card_details_and_checkbox() {
        ...
        let sut = AddPaymentMethodViewController(intent: intent, elementsSession: ._testValue(intent: intent), configuration: config, previousCustomerInput: previousCustomerInput, formCache: .init())
