...
class EmbeddedPaymentElementTest: XCTestCase {
    ...
    func testChangeButtonStateRespectsCardBrandChoice() async throws {
        ...
        if let panConfig = cardForm.getTextFieldElement("Card number").configuration as? TextFieldElement.PANConfiguration,
           let cardBrandChoiceElement = panConfig.cardBrandChoiceElement {
            if let dropdown = cardBrandChoiceElement.dropdownElement {
                ...
                // New segmented selector mode
                let brandToSelect = STPCardBrand.cartesBancaires.makeCardBrandItem()
                selector.select(brandToSelect, shouldAutoAdvance: false)
            }
            ...
            // CBC is not enabled, skip the brand selection test
            print("CardBrandChoiceElement not present, skipping brand selection test")
        }
    }
