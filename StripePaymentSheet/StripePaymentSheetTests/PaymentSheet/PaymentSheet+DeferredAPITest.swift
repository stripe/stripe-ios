...
final class PaymentSheet_DeferredAPITest: STPNetworkStubbingTestCase {
    ...
    // MARK: Helpers
    func createValidSavedPaymentMethod() async throws -> STPPaymentMethod {
        return try await withCheckedThrowingContinuation { continuation in
            apiClient.createPaymentMethod(with: ._testValidCardValue()) { paymentMethod, error in
                if let paymentMethod = paymentMethod {
                    ...
                    continuation.resume(throwing: error ?? NSError(domain: "TestError", code: -1, userInfo: [NSLocalizedDescriptionKey: "Failed to create payment method"]))
                }
            }
        }
    }
    ...
    func testHandleDeferredIntentConfirmation_withPreparePaymentMethodHandler_setupMode() async throws {
        // Given
        let testPaymentMethod = try await createValidSavedPaymentMethod()
        let handlerCalledExpectation = expectation(description: "PreparePaymentMethodHandler called")
    ...
    func testHandleDeferredIntentConfirmation_withoutPreparePaymentMethodHandler_proceedsToNormalFlow() async throws {
        // Given
        let testPaymentMethod = try await createValidSavedPaymentMethod()
    ...
    func testHandleDeferredIntentConfirmation_withPreparePaymentMethodHandler_doesNotProceedToIntentConfirmation() async throws {
        // Given
        let testPaymentMethod = try await createValidSavedPaymentMethod()
        let handlerCalledExpectation = expectation(description: "PreparePaymentMethodHandler called")
    ...
    func testPMOSetupFutureUsageSetsMandateDataForSatispay() async throws {
        // This test verifies that when IntentConfiguration has PMO setup_future_usage for Satispay,
        // mandate_data is properly set even if the retrieved PaymentIntent doesn't have it.
        // This is the bug scenario reported by users with EmbeddedPaymentElement.

        // Create a Satispay payment method params
        let satispayParams = STPPaymentMethodParams(
            satispay: STPPaymentMethodSatispayParams(),
            billingDetails: nil,
            metadata: nil
        )

        // Create IntentConfiguration with PMO setup_future_usage for Satispay
        let intentConfig = PaymentSheet.IntentConfiguration(
            mode: .payment(
                amount: 1099,
                currency: "EUR",
            ...
            // Create a PaymentIntent WITHOUT setup_future_usage or PMO setup_future_usage
            // This simulates the bug scenario where the backend PI doesn't have SFU set
            let clientSecret = try await STPTestingAPIClient.shared.fetchPaymentIntent(
                ...
                merchantCountry: "IT",
                customerID: nil
                // CRITICAL: NOT passing setup_future_usage here
            )
            return clientSecret
        }
        ...
        let result = await PaymentSheet.routeDeferredIntentConfirmation(
            confirmType: .new(
                params: satispayParams,
                paymentOptions: .init(),
                paymentMethod: nil,
                shouldSave: false,
                shouldSetAsDefaultPM: nil
            ),
            configuration: configuration,
            intentConfig: intentConfig,
            authenticationContext: TestAuthenticationContext(),
        ...
        // Verify the payment succeeded (or at least got past mandate_data validation)
        // If mandate_data is missing, the API will reject the request
        switch result.result {
        case .completed, .canceled:
            // Success - mandate_data was properly set
            break
        case .failed(let error):
            // Check if it's a mandate_data error
            let nsError = error as NSError
            if nsError.domain == "com.stripe.lib" && nsError.localizedDescription.contains("mandate") {
                XCTFail("Mandate data was not properly set: \(error.localizedDescription)")
            }
            // Other errors are acceptable (e.g., test payment method issues)
        }
    }
}
