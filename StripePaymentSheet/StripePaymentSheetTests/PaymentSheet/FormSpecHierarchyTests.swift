...
// MARK: - Mandate Tests (for payment methods that show mandate when setting up)

class FormSpecMandateTests: XCTestCase {
    ...
    /// Creates a factory with isSettingUp control using the required initializer
    private func makeFactory(
        for paymentMethodType: STPPaymentMethodType,
        isSettingUp: Bool
    ) -> PaymentSheetFormFactory {
        ...
        return PaymentSheetFormFactory(
            ...
            allowsLinkDefaultOptIn: false,
            forceSaveFutureUseBehavior: false,
            signupOptInFeatureEnabled: false,
            signupOptInInitialValue: false,
            isFirstSavedPaymentMethod: true,
            analyticsHelper: nil,
            paymentMethodIncentive: nil,
            linkAppearance: nil,
            sellerName: nil,
            previousLinkInlineSignupAction: nil,
            cardFundingFilter: .default
        )
    }

    func testCashAppShowsMandateWhenSettingUp() throws {
        let factory = makeFactory(for: .cashApp, isSettingUp: true)
        let form = factory.make()

        let allElements = form.getAllUnwrappedSubElements()
        let hasMandateElement = allElements.contains { $0 is SimpleMandateElement }
        XCTAssertTrue(hasMandateElement, "CashApp should show mandate when setting up")
    }

    func testCashAppHidesMandateWhenNotSettingUp() throws {
        let factory = makeFactory(for: .cashApp, isSettingUp: false)
        let form = factory.make()

        let allElements = form.getAllUnwrappedSubElements()
        let hasMandateElement = allElements.contains { $0 is SimpleMandateElement }
        XCTAssertFalse(hasMandateElement, "CashApp should not show mandate when not setting up")
    }

    func testPayPalShowsMandateWhenSettingUp() throws {
        let factory = makeFactory(for: .payPal, isSettingUp: true)
        let form = factory.make()

        let allElements = form.getAllUnwrappedSubElements()
        let hasMandateElement = allElements.contains { $0 is SimpleMandateElement }
        XCTAssertTrue(hasMandateElement, "PayPal should show mandate when setting up")
    }

    func testRevolutPayShowsMandateWhenSettingUp() throws {
        let factory = makeFactory(for: .revolutPay, isSettingUp: true)
        let form = factory.make()

        let allElements = form.getAllUnwrappedSubElements()
        let hasMandateElement = allElements.contains { $0 is SimpleMandateElement }
        XCTAssertTrue(hasMandateElement, "Revolut Pay should show mandate when setting up")
    }

    func testKlarnaShowsMandateWhenSettingUp() throws {
        let factory = makeFactory(for: .klarna, isSettingUp: true)
        let form = factory.make()

        let allElements = form.getAllUnwrappedSubElements()
        let hasMandateElement = allElements.contains { $0 is SimpleMandateElement }
        XCTAssertTrue(hasMandateElement, "Klarna should show mandate when setting up")
    }

    func testAmazonPayShowsMandateWhenSettingUp() throws {
        let factory = makeFactory(for: .amazonPay, isSettingUp: true)
        let form = factory.make()

        let allElements = form.getAllUnwrappedSubElements()
        let hasMandateElement = allElements.contains { $0 is SimpleMandateElement }
        XCTAssertTrue(hasMandateElement, "Amazon Pay should show mandate when setting up")
    }

    func testSatispayShowsMandateWhenSettingUp() throws {
        let factory = makeFactory(for: .satispay, isSettingUp: true)
        let form = factory.make()

        let allElements = form.getAllUnwrappedSubElements()
        let hasMandateElement = allElements.contains { $0 is SimpleMandateElement }
        XCTAssertTrue(hasMandateElement, "Satispay should show mandate when setting up")
    }
}
