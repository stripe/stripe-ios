...
typedef enum STPErrorCode {
    // These NSError codes are used in the errors returned by the API request.  See https://stripe.com/docs/api?lang=curl#errors for more information about these errors.
    STPInvalidRequestError = 50,
    STPAPIError = 60,

    // STPCardErrors are returned both by the API and by failed client-side validations in STPCard.
    STPCardError = 70
} STPErrorCode;

// The parameter that an NSError returned by Stripe is for.  Corresponds to the "param" property returned in errors from the Stripe API.
FOUNDATION_EXPORT NSString * const STPErrorParameterKey;

// Stripe API error messages are intended to be developer-facing, not customer-facing, so this key holds the message.  Corresponds to the "message" property returned in errors from the Stripe API.
FOUNDATION_EXPORT NSString * const STPErrorMessageKey;

// All NSErrors that have a code of STPCardError will have an STPCardErrorCodeKey in the userInfo dictionary
FOUNDATION_EXPORT NSString * const STPCardErrorCodeKey;

// These are possible values for the STPCardErrorCodeKey in the userInfo dictionary of an NSError returned by this library

// These four values may be returned for STPCardErrorCodeKey by the client-side validations done in STPCard as well as by the call to createTokenWithCard
...
// These values may be returned for STPCardErrorCodeKey from the call to createTokenWithCard
...
// These define user-facing, localizable error messages in all NSErrors returned  by this library. We use macros instead of string constants so that you can use genstrings to generate your Localizable.strings file
