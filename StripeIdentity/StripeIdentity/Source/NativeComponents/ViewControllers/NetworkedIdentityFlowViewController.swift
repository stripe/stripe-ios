...
final class NetworkedIdentityFlowViewController: UIViewController {
    ...
    enum Copy {
        ...
        static let savePreparedTitle = "You saved your ID"
        static let savePreparedBody = "Manage your saved IDs on Link."
    ...
    init(coordinator: NetworkedIdentityCoordinator, includesSelfie: Bool) {
        ...
        // Link's own sheet draws its header without iOS 26's glass capsules; match it.
        #if compiler(>=6.2)
    ...
    func render(_ state: NetworkedIdentityState) {
        ...
        switch state {
        case .idle, .fullCaptureFallback, .cancelled, .linkAuthentication:
            break
        case .preparing:
        ...
        case .skipPending:
            renderProgress("Continuing with manual verification…")
        case .savePending:
            renderProgress(Copy.saving)
        case .savePrepared:
            renderResult(
                step: .savePrepared,
                title: Copy.savePreparedTitle,
                body: Copy.savePreparedBody,
                buttonText: Copy.done
            )
...
private extension NetworkedIdentityFlowViewController {
    ...
    func renderResult(step: VisibleStep, title: String, body: String, buttonText: String = Copy.continueButton) {
    ...
    func manualCaptureButtons() -> [IdentityFlowView.ViewModel.Button] {
        guard coordinator.mode == .reuse, coordinator.state != .skipPending else {
            return []
        }
        return [
            .init(
