...
* The `Stripe` module is now split between `StripeElements`, `StripePayments`, and `StripePaymentsUI`. Some manual changes may be required. Migration instructions are available at [https://stripe.com/docs/mobile/ios/sdk-23-migration](https://stripe.com/docs/mobile/ios/sdk-23-migration).
* [Changed] If you use PaymentSheet, you must now `import StripeElements`. PaymentSheet users no longer need to import the `Stripe` module.
