...
fileprivate class _stpinternal_JSONDecoder: Decoder, STPDecodingContainerProtocol {
    var userInfo: [CodingUserInfoKey : Any] = [:]
    var codingPath: [CodingKey] = []
    var jsonObject: NSObject
    
    init(jsonObject: NSObject) {
    ...
    func container<Key>(keyedBy type: Key.Type) throws -> KeyedDecodingContainer<Key> where Key : CodingKey {
        guard let dict = jsonObject as? NSDictionary else {
            throw DecodingError.typeMismatch(NSDictionary.self, .init(codingPath: codingPath, debugDescription: "KeyedContainer is not a dictionary", underlyingError: nil))
        }
        return KeyedDecodingContainer<Key>(STPKeyedDecodingContainer(codingPath: codingPath, dict: dict, allKeys: [], userInfo: userInfo))
    }
    ...
    func singleValueContainer() throws -> SingleValueDecodingContainer {
        return STPSingleValueDecodingContainer(codingPath: codingPath, userInfo: userInfo, object: jsonObject)
    }
}

fileprivate protocol STPDecodingContainerProtocol {
    var userInfo: [CodingUserInfoKey: Any] {
        get set
    }
}

fileprivate struct STPKeyedDecodingContainer<K>: STPDecodingContainerProtocol, KeyedDecodingContainerProtocol where K: CodingKey {
    var codingPath: [CodingKey]
    ...
    func _objectForKey(_ key: K) throws -> NSObject {
        if !contains(key) {
            throw DecodingError.keyNotFound(key, .init(codingPath: codingPath, debugDescription: "Key \(key) not found in \(codingPath)", underlyingError: nil))
        }
    ...
    func nestedContainer<NestedKey>(keyedBy type: NestedKey.Type, forKey key: K) throws -> KeyedDecodingContainer<NestedKey> where NestedKey : CodingKey {
        ...
        return KeyedDecodingContainer<NestedKey>(STPKeyedDecodingContainer<NestedKey>(codingPath: [], dict: NSMutableDictionary(), allKeys: [], userInfo: [:]))
    }
    
    func nestedUnkeyedContainer(forKey key: K) throws -> UnkeyedDecodingContainer {
        assertionFailure("nestedUnkeyedContainer(forKey:) is not implemented.")
        return STPUnkeyedDecodingContainer(userInfo: [:], array: NSArray(), codingPath: [], currentIndex: 0)
    }
...
fileprivate struct STPUnkeyedDecodingContainer: UnkeyedDecodingContainer, STPDecodingContainerProtocol {
    var userInfo: [CodingUserInfoKey : Any]
...
fileprivate struct STPSingleValueDecodingContainer: SingleValueDecodingContainer, STPDecodingContainerProtocol {
    var codingPath: [CodingKey]
...
fileprivate protocol _STPDecodableIsArray {
...
fileprivate protocol _STPDecodableIsDictionary {
    static var valueType: Decodable.Type { get }
}
...
fileprivate extension Decodable {
    static func _castFromNSObject(codingPath: [CodingKey] = [], decodingContainer: STPDecodingContainerProtocol, object: NSObject) throws -> Self {
...
extension STPDecodingContainerProtocol {
    func castFromNSObject<T>(codingPath: [CodingKey] = [], _ type: T.Type, _ object: NSObject) throws -> T where T: Decodable {
        switch type {
        case is Double.Type:
            ...
            case .none, .some(_):
                guard let value = object as? Double, let returnValue = value as? T else  {
                    throw DecodingError.dataCorrupted(.init(codingPath: codingPath, debugDescription: "Parsed JSON number <\(object)> does not fit in \(type).", underlyingError: nil))
                }
                return returnValue
            }
        case is Float.Type:
        ...
        case is Decimal.Type:
            guard let decimal = (object as? NSNumber)?.decimalValue else {
                throw DecodingError.dataCorrupted(.init(codingPath: codingPath, debugDescription: "Could not convert <\(object)> to \(type).", underlyingError: nil))
            }
            return decimal as! T
        case is URL.Type:
        ...
        case is _STPDecodableIsDictionary.Type:
            ...
            for (k, v) in dict {
                ...
                convertedDict[k] = try dictType.valueType._castFromNSObject(codingPath: codingPath, decodingContainer: self, object: v as! NSObject)
            }
