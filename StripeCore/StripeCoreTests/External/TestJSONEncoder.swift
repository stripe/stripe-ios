...
//        XCTAssertThrowsError(try JSONDecoder().decode(Decimal.self, from: "100e200".data(using: .utf8)!))
    ...
    func test_codingOfUIntMinMax() {
        ...
        do {
            ...
            compareJSON(r, "{\"uint64_min\":0,\"uint64_max\":18446744073709551615,\"int64_min\":-9223372036854775808,\"int64_max\":9223372036854775807}")
        } catch {
...
func expectEqualPaths(_ lhs: [CodingKey?], _ rhs: [CodingKey?], _ prefix: String) {
    if lhs.count != rhs.count {
        XCTFail("\(prefix) [CodingKey?].count mismatch: \(lhs.count) != \(rhs.count). \(lhs) != \(rhs)")
        return
    }
...
struct Numbers : Codable, Equatable {
    ...
    init(from decoder: Decoder) throws {
        ...
        guard decodedValues == values else {
            throw DecodingError.dataCorrupted(DecodingError.Context(codingPath: decoder.codingPath, debugDescription: "The Numbers are wrong! decoded \(decodedValues) but expected \(values)!"))
        }
    }
